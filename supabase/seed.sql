...
--   positions            5
--   topics              24 (hierarchical)
--   questions           53 canonical (10 base + 24 π-series + 19 LLM/embodied
--                         八股 supplements, see questions blocks)
...
insert into public.topics (id, name, slug, parent_id, description) values
  ...
  ('d1000000-0000-4000-8000-000000000015', 'π 系列',           'pi-series',        'd1000000-0000-4000-8000-000000000001', 'Physical Intelligence 的通用机器人基础模型家族：π0、π-FAST、π0.5、π*0.6、π0.7 与 RLT。'),
  ('d1000000-0000-4000-8000-000000000016', 'Flow Matching',    'flow-matching',    'd1000000-0000-4000-8000-000000000006', '学习速度场把噪声分布输运为动作分布的生成建模方法，π0 的动作生成核心。'),
  ('d1000000-0000-4000-8000-000000000017', 'Action Chunking',  'action-chunking',  'd1000000-0000-4000-8000-000000000006', '一次预测一段未来动作并滚动执行的控制方式。'),
  ('d1000000-0000-4000-8000-000000000018', '动作 Tokenization', 'action-tokenization', 'd1000000-0000-4000-8000-000000000006', '把连续动作序列离散化为 token 的方法，如 FAST 的 DCT+BPE。'),
  ('d1000000-0000-4000-8000-000000000019', '参数高效微调', 'peft',              'd1000000-0000-4000-8000-000000000002', 'LoRA、Adapter 等冻结主干、训练低秩增量的微调范式。'),
  ('d1000000-0000-4000-8000-000000000020', '分布式训练',       'distributed-training', 'd1000000-0000-4000-8000-000000000002', '数据并行、张量并行、流水并行与 ZeRO/FSDP 等大模型训练策略。'),
  ('d1000000-0000-4000-8000-000000000021', '对齐',             'alignment',        'd1000000-0000-4000-8000-000000000002', 'SFT、RLHF、DPO、GRPO 等让模型行为符合人类意图的方法族。'),
  ('d1000000-0000-4000-8000-000000000022', '模仿学习',         'imitation-learning', 'd1000000-0000-4000-8000-000000000001', '从专家示范中学习策略的行为克隆及其改进（ACT、DAgger 等）。'),
  ('d1000000-0000-4000-8000-000000000023', 'Sim2Real',        'sim2real',         'd1000000-0000-4000-8000-000000000001', '仿真训练策略向真实机器人迁移的差距来源与弥合方法。'),
  ('d1000000-0000-4000-8000-000000000024', '评测与基准',       'evaluation-benchmarks', 'd1000000-0000-4000-8000-000000000001', '衡量模型能力的方法与基准：LLM 评测指标与具身操作基准。');
...
-- ---------------------------------------------------------------------------
-- questions (π 系列高频 24 题)
--
-- Sourced from bo233 的《具身智能面经（一）：π 论文篇，从 π0 到 π-FAST 的
-- 24 个高频问题》(知乎 https://zhuanlan.zhihu.com/p/2081126347766875774，
-- 微信公众号「深蓝职通车」2026-09-20 转载)。Question wordings follow the
-- article verbatim; canonical/deep answers are curated from the π-series
-- papers. These are authored directly in Chinese (the article language), so
-- unlike the block above they are not rewritten by the Chinese display
-- update further down.
-- ---------------------------------------------------------------------------
...
insert into public.questions (id, title, slug, question_type, difficulty, summary, canonical_answer, deep_answer) values
  (
    ...
    'π0 在 VLM 主干、视觉编码器以及图像与语言 token 融合方面有哪些特殊设计？注意力如何交互？',
    'pi0-vlm-backbone-and-multimodal-token-fusion',
    'knowledge', 'medium',
    'π0 直接沿用 PaliGemma（SigLIP 视觉编码器 + Gemma 语言主干），多相机图像 patch 与语言 token 拼接共享全注意力；机器人专有信号不进入 VLM 上下文，只经输入投影提供给动作专家。',
    'π0 直接以 PaliGemma 3B 为骨干：SigLIP 把多相机图像编码成视觉 patch token，语言指令 tokenize 后与视觉 token 拼接为同一序列，在 Gemma 主干中做全注意力融合，语言与图像之间是标准的双向注意力。机器人特有信号（本体状态、噪声动作）被隔离在 VLM 上下文之外，只通过额外的输入投影进入动作专家，以最大限度保留互联网图文预训练知识。',
    '回答的加分点是知识隔离（knowledge insulation）：动作与状态的分布很窄、与语言分布差异大，若直接进入 VLM 上下文会把主干带偏。π0 的做法是 VLM 只处理图像和语言，动作专家（约 300M 的小 Transformer）与主干联合训练，动作 token attend 到 VLM 的表示以获取语义条件，而 VLM token 无需 attend 回动作 token。对比方案包括 cross-attention 注入、Q-Former 式压缩读取等，可以顺势讨论各自的迁移性与实现成本。'
  ),
  (
    'f1000000-0000-4000-8000-000000000012',
    'π0 的 VLM 输出如何注入 Flow Matching 动作模块？',
    'pi0-how-vlm-output-feeds-flow-matching-action-expert',
    'knowledge', 'medium',
    '动作专家从高斯噪声动作块出发，每步依据 VLM 提供的上下文 KV 预测速度场，沿 ODE 积分若干步得到干净动作块。',
    'Flow matching 头把一个动作块初始化为高斯噪声，每一步根据当前噪声动作与上下文预测速度场，再沿学到的向量场积分数步，把噪声输运为连续动作分布的样本。VLM 的角色是提供条件：动作专家是与主干共享注意力的双流 Transformer，动作 token 对 VLM 的图像/语言 token 做注意力，从而读取语义与观测信息；训练目标只是回归给定噪声水平下的目标速度。',
    '常见追问包括：为什么选 flow matching 而不是 diffusion——目标更简单（向量场回归）、推理步数灵活可调（约 10 步）、比离散自回归更适合高频连续控制；以及与 diffusion policy 的区别（flow matching 学的是确定性输运路径，训练更稳定）。还可以提到动作专家小于主干带来的推理加速：重计算发生在小网络上，VLM 每次重规划只需前向一次。'
  ),
  (
    'f1000000-0000-4000-8000-000000000013',
    'π0 中的 action chunk 具体表示哪些时间范围与控制量？模型如何生成并在执行时滚动使用这一段动作？',
    'pi0-action-chunk-horizon-and-rolling-execution',
    'knowledge', 'medium',
    '动作块覆盖未来约 1 秒（H=50、50Hz）的关节级控制量；块内按时间戳顺序执行，执行期间或块尾重新观测并生成下一块，相邻块用外推与平滑衔接。',
    'π0 输出 H=50 步、以 50Hz 控制的动作块，约对应 1 秒的低层控制量（关节位置/速度级）。执行时机器人按时间戳回放块内动作，同时后台启动下一次推理：π0 用 inference-time action extrapolation，在旧块尚未执行完时先基于当前观测外推出新块，再拼接执行，从而隐藏推理延迟；块与块之间配合时间平滑，避免边界抖动。',
    'chunk 长度本质是延迟与反应性的权衡：太短则高频推理不可行且动作抖动，太长则对动态物体和接触变化的反应变慢。面试可以延伸到：chunk 边界的重规划频率决定了闭环带宽；接触密集任务需要更短 chunk 或外环阻抗控制兜底；以及 ACT 的 temporal ensembling 与 π0 外推策略的异同。'
  ),
  (
    'f1000000-0000-4000-8000-000000000014',
    '若既要利用预训练 VLM，又要对齐连续动作，除了梯度隔断外还有哪些可行的设计？',
    'aligning-pretrained-vlm-with-continuous-actions',
    'knowledge', 'hard',
    '可选：冻结主干只训 adapter/动作头、LoRA 参数高效微调、两阶段离散预训练后接连续专家、完全离散自回归输出、并行读出头等，各自在迁移与控制精度间取舍。',
    '常见方案：1）冻结 VLM，仅训练 adapter 或动作头，迁移最好但控制上限受主干表示约束；2）LoRA 等参数高效微调，折中保护预训练知识；3）π0.5 式两阶段——先用离散动作 token 与网络数据共同预训练，再接入 flow matching 专家微调；4）π-FAST 式完全离散化，让 VLM 直接自回归生成动作 token；5）在主干之上加并行读出头直接回归动作。',
    '回答框架是迁移能力与控制精度的权衡：梯度全回传最灵活但易灾难性遗忘、训练昂贵；离散化迁移最好但受 token 化精度与序列长度限制；冻结方案省算力却牺牲适配。面试官通常追问各方案失败模式——遗忘、量化误差、推理延迟——以及这些失败模式如何对应 π 系列不同版本的选择。'
  ),
  (
    'f1000000-0000-4000-8000-000000000015',
    'π0.5 的网络架构、输入与输出、训练损失和关键设计分别是什么？',
    'pi05-architecture-inputs-outputs-training-loss',
    'knowledge', 'medium',
    '架构延续 π0 的 VLM + 动作专家；推理先预测高层子任务文本，再以子任务为条件生成动作；训练用异构 co-training，动作预训练混入 FAST 离散 token，损失含子任务语言建模与动作项。',
    'π0.5 沿用 π0 的 VLM + flow matching 专家架构，但推理变成两级：先由模型生成下一阶段子任务的自然语言 token，再以该子任务为条件生成连续动作。训练采用 co-training：机器人演示的动作损失、高层子任务预测的语言建模损失、网络图文数据的 VQA 损失联合优化；低层动作在预训练阶段以 FAST 离散 token 表示，最终微调回到 flow matching 专家。',
    '关键设计的动机值得展开：子任务预测给模型显式的高层规划接口，把长程任务分解成可监督的中间目标；异构 co-training 是把开放世界泛化「买」进来的主要手段。可以对比 π0：架构相似，改变的主要是数据配方与训练目标，这也引出「π0.5 是数据配方的胜利」这一常见总结。'
  ),
  (
    'f1000000-0000-4000-8000-000000000016',
    'π0 与 π0.5 在核心目标、数据与动作表示、训练方式和泛化能力上有哪些区别？',
    'difference-between-pi0-and-pi05',
    'knowledge', 'medium',
    'π0 主打通用高频低层控制，π0.5 主打开放世界泛化：靠异构 co-training、子任务预测与离散 token 预训练 + 连续专家微调的两阶段训练，把泛化能力显著推高。',
    '核心目标：π0 解决通用机器人的高频灵巧低层控制；π0.5 解决开放世界（全新环境、全新家庭）泛化。数据与动作表示：π0 用多机器人演示加连续 flow matching；π0.5 混入网络图文数据、子任务标注与言语指令，动作在预训练阶段用 FAST 离散 token。训练方式：π0 直接端到端 flow matching 训练；π0.5 先离散 token co-training，再接连续专家微调，并显式训练子任务预测。泛化能力：π0.5 能在训练中从未见过的家庭里执行长程家务任务。',
    '一个好用的一句话框架：「π0 是动作专家的胜利，π0.5 是数据配方的胜利」。追问常常是两阶段训练为什么两头收益——离散 token 训练效率高、与语言数据同构利于迁移，连续专家保证推理质量与高频控制；以及泛化的边界：语义层面泛化强，精细操作精度仍依赖任务内微调。'
  ),
  (
    'f1000000-0000-4000-8000-000000000017',
    'π0 与 π0.5 如何处理 VLA 中的机器人本体状态？这种设计差异会带来哪些影响？',
    'pi0-vs-pi05-proprioception-handling',
    'knowledge', 'medium',
    '两者都把本体状态经输入投影只喂给动作专家而非 VLM，保持主干本体无关；π0.5 更强调跨本体共训，状态的作用被进一步弱化，更多依赖视觉闭环。',
    '两个模型都遵循知识隔离：关节角度等本体状态不进入 VLM 的语言/图像 token 序列，只通过独立的输入投影提供给动作专家。这使 VLM 的表征不被窄分布的本体数据污染，跨本体迁移更稳。π0.5 进一步强调跨机器人共训与视觉闭环，弱化了对精确本体读数的依赖；需要本体反馈的接触密集任务则保留动作专家这条通道。',
    '影响可以双向讨论：若把本体状态放进 VLM，单本体精度可能提升，但破坏迁移且语言 token 与连续状态的分布冲突；隔离的代价是主干缺少本体感知，接触密集、盲区操作场景必须依赖动作专家。还可以提到训练技巧如 state dropout（随机丢弃状态输入迫使模型学会无状态兜底），以及跨本体数据集对本体观测空间不一致的处理。'
  ),
  (
    'f1000000-0000-4000-8000-000000000018',
    'π0.5 的 Action Expert 梯度会回传至 VLA 吗？为什么？',
    'does-pi05-action-expert-gradient-flow-back-to-vlm',
    'knowledge', 'medium',
    '会。动作专家与 VLM 端到端联合训练，动作损失经注意力反传到主干；预训练知识靠海量 co-training 数据与小学习率保护。',
    '会回传。π0/π0.5 中动作专家与 VLM 联合端到端训练：flow matching 或 token 损失对动作专家求导后，经共享注意力继续向 VLM 主干传播，机器人相关的输入投影也同步更新。设计者选择全参微调而非隔断，是因为任务要求主干真正理解机器人语境；预训练知识由异构 co-training 数据与训练超参保护，而不是靠梯度隔断。',
    '对比方向：冻结主干（如 π-FAST 之上只训 RL 头、RLT 冻结 VLA 只训 actor-critic 头）保护知识但上限受既有表示约束；梯度回传上限高但数据不足时会遗忘。面试加分点是指出「梯度是否隔断」不是教条，而是数据规模、算力预算与任务分布偏移程度共同决定的工程决策。'
  ),
  (
    'f1000000-0000-4000-8000-000000000019',
    'π0.5 的开放世界泛化由哪些数据、训练目标和架构设计共同支撑？这些因素分别如何起作用？',
    'what-supports-pi05-open-world-generalization',
    'knowledge', 'hard',
    '数据层的异构 co-training（网络图文、多机器人、言语指令）、训练目标层的高层子任务预测、训练策略层的离散 token 预训练 + 连续专家微调，三层叠加支撑开放世界泛化。',
    '三个层面：数据上，网络图文/视频数据与多机器人演示共同训练，把语义常识注入主干，这是全新家庭泛化的主因；训练目标上，高层子任务预测把长程任务变成「先想后做」，缓解分布外任务的组合爆炸；训练策略上，离散 token 预训练与 flow matching 专家微调的两阶段设计兼顾训练效率与推理质量。推理时模型先分解任务（如「打开水槽下的柜门」）再执行，使新环境中行为仍有结构。',
    '面试官常追问「哪一项最关键」：论文证据指向非机器人数据的 co-training 是主要驱动力，子任务预测与两阶段训练是放大器。还应主动给出泛化边界：语义与流程泛化强，但操作精度在分布外环境仍会下降，需要任务内数据闭环兜底。'
  ),
  (
    'f1000000-0000-4000-8000-000000000020',
    'π0.5 中用于高层子任务或机器人规划的模块负责预测什么信息？它如何为低层连续动作生成提供条件？',
    'pi05-high-level-subtask-module',
    'knowledge', 'medium',
    '该模块预测下一阶段子任务的自然语言 token；推理时先采样子任务，再把它与观测拼接作为动作专家的条件，随执行进度滚动更新。',
    '高层模块就是共享 VLM 的语言读出：给定图像、指令与历史，预测「下一步做什么」的子任务文本 token。推理时先推断子任务，再把子任务 token 与观测一起作为条件驱动动作专家生成连续动作块；随着执行推进，子任务被重新预测和更新，形成层级式闭环。训练上由带子任务标注的异构数据以语言建模损失监督，与动作损失共同优化。',
    '设计收益：显式中间目标提升可解释性（人可以读出甚至纠正子任务）、使低层条件分布更聚焦、把长程任务分解为短程问题。追问点包括：子任务粒度如何确定、错误子任务如何恢复（重新预测的鲁棒性）、以及与 RT-2 式「直接把动作当语言输出」的层级化区别。'
  ),
  (
    'f1000000-0000-4000-8000-000000000021',
    'π*0.6 要解决什么问题？为什么大规模 VLA 不易直接采用 PPO？后训练如何转化为近似监督学习？',
    'pistar06-why-not-direct-ppo',
    'knowledge', 'hard',
    'π*0.6 要让 VLA 从自身经验（含失败）中变得可靠；真机 PPO 受采样成本、稀疏奖励与 on-policy 约束限制；RECAP 用离线 value 学习 + 优势条件化训练把 RL 转化为条件化监督学习。',
    'π*0.6 的目标是后训练可靠性：让机器人从自主收集的正负经验中持续提高成功率。不直接用 PPO 的原因：真机采样昂贵且慢，on-policy 要求不断用最新策略重采数据；任务奖励稀疏导致长程信用分配方差大；探索性更新在物理硬件上有安全风险。π*0.6 的 RECAP 把 RL 重写为监督问题：在离线经验回放（含失败与人工纠正）上用时间一致性拟合 value 函数，把量化的优势标签作为条件输入策略联合训练；部署时以高优势条件自举，策略学会偏好高优势动作。',
    '追问点：value 回归为什么可行——回放数据分布固定，时序一致性自举等价于拟合离线回报；与 Decision Transformer 式条件化的亲缘关系；以及为什么仍需要人工纠正数据兜底稀疏奖励。能对比「在线策略梯度」与「离线优势加权」两条技术路线的成本结构是加分项。'
  ),
  (
    'f1000000-0000-4000-8000-000000000022',
    '在 π0.5、π*0.6 与 RLT 等方法中，replay buffer 在数据闭环和策略更新中承担什么作用？',
    'role-of-replay-buffer-in-pi-post-training',
    'knowledge', 'medium',
    'replay buffer 持久化自主经验供反复离线更新：为 value/优势估计提供固定分布、重标注与再平衡、混合新旧数据防止遗忘，是经验闭环类方法的枢纽。',
    '三个作用：1）把真机滚动产生的成败经验持久化，供 RECAP、RLT 这类离线/半离线更新反复采样，摊薄昂贵的采集成本；2）为 value 与优势估计提供相对固定的数据分布，使时序自举稳定收敛；3）把旧演示、人工纠正与新经验按比例混合与再平衡，防止策略更新偏移或遗忘。π0.5 的异构 co-training 虽不是 RL，也共享同一套「缓存-再平衡-重加权」的数据闭环思想。',
    '延伸讨论：FIFO 与优先级采样对分布的影响、成败样本配比如何左右 value 的校准、以及「buffer 分布等于部署分布」这一隐含假设在环境变化时失效的后果。这题考察的是把 RL 组件放回数据闭环里理解的能力，而不是背概念。'
  ),
  (
    'f1000000-0000-4000-8000-000000000023',
    'π*0.6 的 reward model 如何获得奖励信号？奖励设计如何支持后训练？',
    'pistar06-reward-model-signal-source',
    'knowledge', 'hard',
    '不依赖手工密集奖励：以少量人工核验的成败标签为种子，在回放数据上用时间一致性自举 value/reward 信号，为全部无标签经验自动打分。',
    '奖励信号有两条来源：一小部分经验由人工核验任务成败，形成种子标签；在此之上于回放数据上训练 value/reward 模型，用时间一致性自举把终局成败的信用沿轨迹向前传播，从而给无标签的成败经验自动打分。奖励刻意保持稀疏、以任务成败为主，避免稠密塑形带来的偏差，把「如何加权经验」交给优势条件化机制完成。',
    '对比方向：仿真中常用的 reward shaping 在跨任务通用后训练中不可扩展，因为每个任务都要人工设计；视觉 reward model 依赖生成或匹配也各有偏差。π*0.6 的取舍是用最小的人工监督 + 自举扩展覆盖面，面试中能说出「稀疏奖励 + 优势条件化」这个组合逻辑即可。'
  ),
  (
    'f1000000-0000-4000-8000-000000000024',
    'π0.7 如何提升组合泛化？（额外上下文、原子指令、目标图像与 world model）',
    'how-pi07-improves-compositional-generalization',
    'knowledge', 'hard',
    'π0.7 用可引导的多模态条件（丰富语言指令、目标/子目标图像、任务元数据）+ 原子子任务分解 + 世界模型生成子目标图像，把新任务重组为已见原子能力的组合。',
    'π0.7（5B：4B VLM 主干 + 视频历史编码器 + 860M 动作专家）把组合泛化建立在更强的「先想后做」上：训练用丰富多样的语言指令与语言引导数据教会原子技能；推理时把长任务分解成原子子任务序列；轻量世界模型为每个子任务生成子目标图像，作为显式视觉条件引导动作专家。多模态条件化（指令、目标图像、元数据）让同一模型可被引导到不同做法与质量档，从而把未见任务表达为已见原子能力的组合。',
    '与 π0.5 的对比是关键：π0.5 的中间产物是文本子任务，π0.7 加入了世界模型生成的子目标图像与可引导性（steerability），组合泛化从「见过类似流程」推进到「重组原子能力」。可以讨论子目标图像的验证作用（执行前先可视化预期）、world model 幻觉的风险，以及 RL 验证训练在精确任务上的角色。'
  ),
  (
    'f1000000-0000-4000-8000-000000000025',
    'RLT 的训练与测试流程是什么？',
    'rlt-training-and-testing-procedure',
    'knowledge', 'medium',
    '在冻结 VLA 上接一个编码-解码 Transformer 抽出压缩的 RL token，再在小 actor-critic 头上做 PPO 风格在线 RL；测试时 RL 头在 token 空间输出对参考动作的修正。',
    '训练：冻结预训练 VLA；插入一个 encoder-decoder Transformer，把 VLA 内部表示压缩为少量 RL token；在 RL token 上接轻量 actor-critic（MLP）头，以 VLA 参考动作为先验、用 PPO 风格的在线更新训练，动作以 chunk 为单位对齐 VLA 原生接口，从而缩短 TD 自举跨度。测试：VLA 前向得到 RL token，RL 头基于它输出对参考动作的修正并执行，新经验继续回流更新 RL 头。',
    '设计动机：对大 VLA 做全参数在线 RL 又贵又危险，RLT 把可学习的部分限制在小头上——冻结的 VLA 提供感知与动作先验，RL 只做局部精修。论文报告在螺钉、扎带、网线、充电器插入等精确操作上用几小时数据同时提升成功率与执行速度（至多 3 倍），可作为具体论据引用。'
  ),
  (
    'f1000000-0000-4000-8000-000000000026',
    '请完整说明 ReCAP 的流程，包括人工介入、训练和测试环节。',
    'recap-full-procedure',
    'knowledge', 'hard',
    '部署采集经验（含人工纠正）→ 回放数据上拟合 value/优势 → 优势条件化策略训练 → 以高优势条件部署执行，新经验回流形成闭环。',
    '1）经验收集：机器人自主滚动任务，出现险情或失败时由人工接管纠正，全部写入回放。2）训练：先做 SFT 恢复基本能力；在回放数据上以时间一致性拟合 value 函数，得到每段经验的优势估计；把优势量化为标签，与状态-动作一起做优势条件化训练（RECAP），人工纠正段天然携带高优势标签。3）测试与部署：策略以「高优势」条件自举执行，持续采集新经验回到第一步，形成部署即学习的闭环。',
    '与在线 RL 的工程差异是回答重点：整个过程中没有环境交互中的梯度更新，学习全部发生在回放数据上的监督式拟合；人工介入既保证安全又提供高质量正例。追问可能在「纠正段的标签为什么可信」「优势条件在部署时如何选择」以及该流程对操作员的依赖程度。'
  ),
  (
    'f1000000-0000-4000-8000-000000000027',
    'ReCAP 与 SFT + GRPO 有什么区别？（数据使用、value/reward 建模和策略更新）',
    'recap-vs-sft-grpo',
    'knowledge', 'hard',
    'GRPO 用组内相对奖励做 on-policy 策略梯度，需要可验证奖励与大规模并行采样；ReCAP 用离线回放上的 value 自举 + 优势条件化监督，不需要在线采样与成组对比。',
    '数据使用：GRPO 对同一任务采样一组 rollout 并组内归一化打分；ReCAP 使用历史回放（含失败与纠正），不要求同任务成组。value/reward 建模：GRPO 依赖可验证奖励的相对比较，不学 value 网络；ReCAP 用时间一致性在离线数据上拟合 value 并导出优势标签。策略更新：GRPO 是 on-policy 策略梯度（clipped 目标加 KL 约束）；ReCAP 把策略更新写成以优势为条件的行为克隆，近似离线 RL。适用面：GRPO 适合可验证、可大规模并行的场景；ReCAP 适合真机、奖励稀疏、采样昂贵的机器人后训练。',
    '共同点是两者都用「相对优势」替代绝对奖励；本质差异是 on-policy 采样对比离线自举。面试中把两条链路各自的数据流画清楚、再点出对方不可行的条件（真机 PPO 类方法贵在采样，GRPO 类方法难在奖励可验证），就是完整答案。'
  ),
  (
    'f1000000-0000-4000-8000-000000000028',
    '与 PPO 相比，RLT 对 value 的监督有什么不同？',
    'rlt-vs-ppo-value-supervision',
    'knowledge', 'hard',
    'PPO 的 critic 在完整观测上逐步 TD/GAE 自举，梯度影响整个策略网络；RLT 的 value 头只读压缩的 RL token，按 chunk 级自举、跨度更短，且 VLA 主干冻结。',
    '标准 PPO：value 网络吃完整观测，逐步 TD 误差加 GAE 自举，value 与 policy 联合更新、梯度可传遍整个网络。RLT：VLA 冻结，value 头只读 RL token——即 VLA 表示的压缩嵌入；监督目标仍是折扣回报，但自举发生在 chunk 边界上，时间跨度更短、方差更小；value 的容量集中在小型 MLP 头上，避免在大 VLA 上学 value 的不稳定性。',
    '延伸点：chunk 级短跨度自举在高频、稀疏奖励的精确操作任务上更稳；MLP critic 的局限是难以表达多模态动作分布下的价值（这也是 RLT 定位为「在 VLA 先验附近精修」的原因）。能对比「value 学在哪、怎么自举、更新谁」三个维度即可拿满。'
  ),
  (
    'f1000000-0000-4000-8000-000000000029',
    'π-FAST 的离散自回归动作生成，与 π0/π0.5 的连续 Flow Matching Action Expert 相比，各有什么训练和推理取舍？在闭环重规划时，两者又分别有什么限制？',
    'pi-fast-vs-flow-matching-action-expert-tradeoffs',
    'knowledge', 'medium',
    '离散 AR 训练快约 5 倍、可复用语言模型基础设施，但推理逐 token 串行；flow expert 推理并行、贴合连续控制，但训练贵。闭环重规划时 AR 每次都要重新生成整段 token，flow expert 靠外推缓解等待。',
    '训练：FAST 把动作压成短离散序列，用标准 next-token 损失，收敛比 flow matching 快约 5 倍，并能与语言数据共享同一套训练设施；flow matching 需要多步去噪式训练与专门实现。推理：AR 生成动作 token 是串行的，chunk 长、控制频率高时延迟压力大；flow expert 少数步积分即可并行产出整块动作。闭环重规划：AR 每次重规划都要重新生成整段 token 序列，块内无法中途修正；flow expert 可以用 inference-time extrapolation 在旧块执行期间预生成新块，重规划延迟更隐蔽。',
    '好答案会给出量化直觉：token 序列长度正比于 chunk 步数乘维度压缩率，控制频率越高串行解码压力越大；工程缓解手段包括 KV cache、投机解码。还应指出 FAST 的定位——加速训练与迁移，而非替代连续专家；π0.5 两阶段并用正是取两者之长。'
  ),
  (
    'f1000000-0000-4000-8000-000000000030',
    'FAST 如何将连续动作序列转换为离散动作 token？与逐维分桶等朴素离散化相比，DCT 与 BPE 分别带来什么变化？',
    'how-fast-tokenizes-actions-with-dct-and-bpe',
    'knowledge', 'medium',
    'FAST 对动作序列沿时间维做 DCT，把能量集中到少数低频系数；量化后用 BPE 把常共现的系数量化为单 token，序列比逐维分桶短一个量级。',
    'FAST 把动作 chunk（时间 × 动作维度矩阵）当作时间序列处理：先对每维动作沿时间做离散余弦变换（DCT），利用相邻动作步的相关性把能量集中到少量低频系数；系数量化后大部分接近零，再用 BPE 在训练集上学习合并规则，把经常共同出现的量化系数合并为单个 token。结果是动作序列长度比朴素方法缩短约一个数量级，训练信号更集中。',
    '对比逐维分桶：分桶忽略时间相关性，token 序列长且冗余，对动作噪声敏感，词表也随维度数增长；DCT 压缩的是时间相关性，BPE 压缩的是系数共现，两者正交叠加。追问可能在 DCT 截断带来的信息损失（衔接下一题）以及词表规模与重构误差的权衡。'
  ),
  (
    'f1000000-0000-4000-8000-000000000031',
    '频域压缩会牺牲哪些动作细节？在什么控制频率、任务精度或闭环要求下，FAST 更适合或不适合？',
    'what-does-frequency-compression-sacrifice',
    'knowledge', 'hard',
    '截断高频系数意味着丢失快速修正与接触瞬态；中低控制频率、动作平滑、以轨迹跟踪为主的任务适合 FAST；高频力控、接触密集、需要瞬时反应的任务不适合。',
    'DCT 截断高频系数牺牲的是动作的快速变化分量：对抖动的抑制是优点，但在插拔、冲击吸收等接触丰富任务中，瞬态力控细节可能丢失，量化还会引入重构误差。适合场景：控制频率中等（约 10-50Hz）、动作平滑、以轨迹跟踪为主的任务，FAST 的训练加速与迁移收益最大。不适合：需要毫秒级高频修正、编码振动或接触瞬态、或动作分布高度多峰的任务。',
    '工程兜底思路值得主动给出：高频反应可以交给外环（阻抗/力控）或提高重规划频率，FAST 只负责中低频轨迹层——π 系列最终的也是这个分工：离散 token 管预训练效率，连续 flow expert 管低层控制质量。能按任务类型给出演示级判断是面试官想看到的。'
  ),
  (
    'f1000000-0000-4000-8000-000000000032',
    'π 系列论文的演进脉络是什么？各版本试图解决的核心问题分别是什么？',
    'pi-series-evolution-line',
    'knowledge', 'medium',
    'π0 确立 VLM+flow expert 架构解决高频灵巧控制 → π-FAST 用离散 token 加速训练 → π0.5 靠异构 co-training 与子任务规划解决开放世界泛化 → π*0.6 用 RECAP 从经验中后训练提可靠性 → π0.7 以可引导条件与世界模型子目标冲组合泛化，RLT 补上高效在线 RL。',
    '演进主线是逐层补短板：π0（2024.10）确立 PaliGemma + flow matching 动作专家架构，解决通用高频低层控制；π-FAST（2025.01）证明 DCT+BPE 离散 token 能把训练提速约 5 倍；π0.5（2025.04）用异构 co-training、子任务预测与两阶段训练解决开放世界泛化；π*0.6（2025.11）以 RECAP/RLPE 让模型从自身经验与人工纠正中后训练，解决真实场景可靠性；π0.7（2026.04）引入可引导多模态条件与世界模型子目标图像，冲击组合泛化；RLT（2026.04）用 RL token 让冻结 VLA 之上的在线 RL 样本高效。',
    '面试加分点是对每一步说出「解决了什么、放弃了什么」：FAST 训练快但牺牲连续表达；π0.5 泛化强但精度仍靠微调；RECAP 可靠但依赖回放分布；RLT 高效但只做先验附近的局部精修。把演进讲成「架构 → 数据 → 后训练 → 可引导」四个阶段最有结构感。'
  ),
  (
    'f1000000-0000-4000-8000-000000000033',
    '如果 π 系列继续推出下一篇工作，你认为最可能的发力方向是什么？',
    'next-direction-for-pi-series',
    'research', 'hard',
    '开放题：可从世界模型与策略深度耦合、在线 RL 闭环自动化、跨本体统一动作接口、长程记忆与移动操作、安全可控生成等角度给出「短板 → 杠杆」的推理链。',
    '合理的候选方向：1）世界模型与策略深度耦合——把 π0.7 的子目标世界模型从条件工具升级为验证器与数据生成器，支撑 safe RL 与经验放大；2）在线 RL 闭环产品化——把 RLT/RECAP 式方法自动化，实现部署即学习；3）跨本体统一动作接口与更大规模真机数据，继续摊薄数据成本；4）长程任务记忆与移动操作（导航与操作的统一策略）；5）形式化安全约束下的可控动作生成。',
    '这道题考的是推理结构而非结论：先指出当前系列的短板（如 π0.7 的世界模型尚未参与训练闭环、RLT 只在小头上学习），再给出对应的技术杠杆与可验证的实验证据设想。能引用系列自己的演进节奏（架构 → 数据 → 后训练 → 可引导）做外推，比罗列热词更有说服力。'
  ),
  (
    'f1000000-0000-4000-8000-000000000034',
    '从动作表示、解码方式和预训练迁移角度比较，π 系列与 RT 系列、OpenVLA 的技术路线有哪些关键差异？',
    'pi-series-vs-rt-series-and-openvla',
    'knowledge', 'medium',
    'π 系列走「VLM + 小动作专家 + flow matching 连续控制」并持续迭代后训练；RT 系列（RT-1/RT-2）与 OpenVLA 把动作离散成 token 自回归输出，侧重大规模示范数据与开源生态。',
    '动作表示：π0/π0.5/π0.7 用连续 flow matching 动作专家；RT-1 用离散化 token，RT-2 与 OpenVLA 把动作当作语言 token 由 VLM 自回归输出。解码与控制频率：连续专家推理并行度高、适配 50Hz 级高频控制；离散 AR 受串行生成延迟约束。预训练迁移：三者都受益于 VLM 预训练，但 π 系列强调知识隔离与异构 co-training 的数据配方；OpenVLA 强调开源数据规模（OXE 混合）与开放权重生态。后训练：π 系列已有 RECAP、RLT 等从经验学习的成套方案，开源路线多停留在 SFT/LoRA 微调。',
    '可以总结成三个维度的「路线之争」：表示（连续 vs 离散）、训练（co-training 配方 vs 数据规模）、后训练（经验闭环 vs 静态权重）。面试中主动给出各自适用场景——研究迭代与高频控制看 π 系列方案，低成本复现与社区生态看 OpenVLA——会显得判断成熟。'
  );

-- ---------------------------------------------------------------------------
-- questions (大模型八股 + 具身高频补充 19 题)
--
-- Sourced from a 小红书 高频问题卡片集（PART 1 大模型八股文 12 题 +
-- PART 2 具身 VLA 18 题，小红书号 49634740272）。11 items overlapping the
-- existing canonical bank (QKV/KV Cache, PPO vs GRPO, π-series evolution,
-- π0.5 vs π0, Action Expert interaction, π-FAST, ReCAP, OpenVLA comparison)
-- were deduped per docs/question-extraction-guidelines.md; the 19 genuinely
-- new concepts below are added with curated answers. Chinese throughout.
-- ---------------------------------------------------------------------------

insert into public.questions (id, title, slug, question_type, difficulty, summary, canonical_answer, deep_answer) values
  (
    'f1000000-0000-4000-8000-000000000035',
    '请推导并解释 Self-Attention 的计算过程，为什么要除以 √dk？',
    'why-scaled-dot-product-attention-divides-by-sqrt-dk',
    'knowledge', 'medium',
    '除以 √dk 是为了控制点积的方差：dk 维点积的方差随维度线性增长，不缩放会让 softmax 饱和、梯度消失。',
    'Self-Attention 先由输入 X 得到 Q = XW_Q、K = XW_K、V = XW_V，再计算 Attention(Q,K,V) = softmax(QK^T / √dk) V：每个 query 与所有 key 做点积得分数，缩放后 softmax 成权重，对 value 加权求和。除以 √dk 的原因是：若 q、k 的分量均值 0 方差 1，则点积 q·k 的方差为 dk，维度越大分数越尖锐，softmax 会趋近 one-hot，梯度趋近于零；除以 √dk 把点积方差拉回约 1，保证 softmax 处于有梯度的区间。',
    '追问延伸：多头场景下 dk = d_model / h，缩放因子随头维度自动调整；softmax 饱和解释是标准答案，可以补充「分数分布过尖 → one-hot → 梯度消失」这条链。还可以提到替代方案（对 logits 做 layernorm、可学习温度）以及 FlashAttention 并不改变这个缩放、只是改变计算顺序。'
  ),
  (
    'f1000000-0000-4000-8000-000000000036',
    'MHA 和 GQA 的区别是什么？各自的优缺点？',
    'mha-vs-gqa',
    'knowledge', 'medium',
    'MHA 每个 query 头有独立 K/V 头，质量最好但 KV Cache 最大；GQA 让一组 query 头共享一个 K/V 头，在质量与显存/带宽之间折中。',
    'Multi-Head Attention 中每个注意力头都有独立的 Q、K、V 投影；Grouped-Query Attention 中多个 query 头共享同一组 K/V 头（每组 G 个 query 头对应 1 个 KV 头），Multi-Query Attention 是 G=所有 query 共享 1 个 KV 头的极端情形。GQA 把 KV Cache 体积和解码带宽需求缩小约 G 倍，精度损失很小，因此 Llama 2/3、Qwen 等主流大模型都采用。',
    '回答框架是「质量 vs KV Cache 容量/带宽」：MHA 质量上限最高、缓存最贵；MQA 缓存最省、质量下降最明显；GQA 是工业界默认折中，且可以从 MHA 权重上变换（mean-pool KV 头）升级得到。具身场景的落点：机载 GPU 上跑 VLA 推理时，KV Cache 常是延迟瓶颈，GQA/量化是第一梯队手段。'
  ),
  (
    'f1000000-0000-4000-8000-000000000037',
    'Batch Normalization、Layer Normalization 和 RMSNorm 的原理与区别？',
    'batchnorm-layernorm-rmsnorm',
    'knowledge', 'medium',
    'BN 沿 batch 维统计每个特征的均值方差，LN 对单样本的特征维归一化，RMSNorm 在 LN 基础上减掉均值中心化只保留 RMS 缩放。',
    'BatchNorm 对每个特征在 batch 维度上计算均值/方差做归一化，训练时维护滑动平均，推理依赖全局统计量。LayerNorm 对每个样本自身在特征维上归一化，不依赖 batch，成为 Transformer 的标准配置（现代模型多用 pre-LN 稳定训练）。RMSNorm 只用均方根做缩放、不减均值，计算更少、效果与 LN 相当，被 LLaMA、Qwen 等主流大模型采用。',
    '高频追问包括：为什么 NLP/Transformer 不用 BN——序列长度可变、小 batch 与分布式统计同步困难，且逐 token 的统计不稳定；pre-LN 与 post-LN 的训练稳定性差异；RMSNorm 省掉了均值项为何几乎不掉点（方向信息主要由相对幅度承载）。机器人策略网络中的归一化也遵循同样的权衡。'
  ),
  (
    'f1000000-0000-4000-8000-000000000038',
    'LoRA 微调的原理是什么？相比全量微调有哪些优缺点？在具身智能中一般如何应用？',
    'lora-principle-and-embodied-applications',
    'knowledge', 'medium',
    '冻结原权重，为部分矩阵学习低秩增量 ΔW = BA；参数与优化器显存大减、可合并可插拔，代价是表达上限略低。',
    'LoRA 冻结预训练权重 W，为其附加低秩分解 ΔW = BA（r 远小于 d），只训练 A、B。参数量、优化器状态和梯度显存大幅下降，多个任务的适配器可以切换或合并回主权重；通常作用于注意力与 MLP 投影矩阵。相比全量微调：优点是省显存、抗遗忘、部署灵活，缺点是秩上限约束了可学到的改动，复杂任务上可能略逊。',
    '具身应用落点：对 VLA 主干做 LoRA 是标准玩法——openpi 的 π0 微调就提供 LoRA 选项，用少量演示数据为不同机器人本体或新任务训练独立适配器，保住 VLM 预训练知识的同时快速适配动作分布。还可以提 QLoRA（4-bit 量化 + LoRA）进一步压显存，以及 rank 选择与目标模块的经验规律。'
  ),
  (
    'f1000000-0000-4000-8000-000000000039',
    '讲一下分布式训练的基本概念？常见的并行策略有哪些（数据并行、模型并行、流水并行等）？',
    'distributed-training-parallelism-strategies',
    'knowledge', 'medium',
    '数据并行复制模型只切 batch，张量并行切开单个算子，流水并行按层分段，ZeRO/FSDP 对参数、梯度、优化器状态分片。',
    '数据并行（DP/DDP）：每个设备持完整模型，切分 batch 后 all-reduce 梯度，简单高效但每个设备要装下完整模型。张量并行（TP）：把单个大矩阵乘切到多卡（Megatron 式按行列切分），通信频繁，通常限定在节点内。流水并行（PP）：按层把模型分段，用 micro-batch 填充流水线减少 bubble，跨节点友好。ZeRO/FSDP 不算新的并行维度，而是把参数、梯度、优化器状态分片到各卡、用时再聚合，大幅降低显存冗余。大模型训练常组合成「三维并行 + 数据并行」。',
    '追问常落在：TP 为什么放节点内（all-reduce 带宽需求）、流水线 bubble 怎么算、ZeRO 1/2/3 分别分片什么、以及通信量与收敛的权衡（梯度累积、bf16 混合精度）。具身关联点：π0.7 级别的 VLA 预训练就是典型的大规模混合并行场景。'
  ),
  (
    'f1000000-0000-4000-8000-000000000040',
    '什么是 Flow Matching？它的核心思想是什么？与扩散模型有什么关系？',
    'what-is-flow-matching',
    'knowledge', 'medium',
    '学习一个速度场把简单分布沿（近似直线的）路径输运到数据分布；训练只是向量场回归，扩散可视为其框架下的特例。',
    'Flow Matching 属于连续归一化流框架：在高斯噪声分布和数据分布之间构造概率路径（常用条件路径 x_t = (1-t)·噪声 + t·数据 的直线插值），训练目标是回归该路径上的速度场 v(x_t, t)，推理时从噪声出发沿学到的 ODE 积分数步采样。与扩散模型的关系：扩散模型可以看作选择特定（弯曲）路径与噪声调度的特例，Flow Matching 的直线路径让训练目标更简单、采样步数更少（Rectified Flow 还能通过 rerun 把路径进一步拉直）。',
    '在具身语境下的落点是 π0：动作专家用 Flow Matching 生成连续动作块，少步 ODE 积分即可满足高频控制。高频追问：与 DDPM/DDIM 的目标差异、为什么回归向量场比预测噪声更稳、以及步数-质量权衡如何调。'
  ),
  (
    'f1000000-0000-4000-8000-000000000041',
    '在大模型训练和推理中，如何进行显存优化？（如梯度检查点、混合精度、ZeRO 等）',
    'gpu-memory-optimization-training-and-inference',
    'knowledge', 'hard',
    '训练侧靠混合精度、梯度检查点、ZeRO 分片、梯度累积和 8-bit 优化器；推理侧靠 KV Cache 量化/分页、权重量化和小头部署。',
    '训练显存的大头是参数、梯度、优化器状态（Adam 约为参数的 4 倍）和激活。手段：bf16/fp16 混合精度（保留 fp32 主权重）省一半以上；梯度检查点用重算换激活显存；ZeRO-1/2/3（或 FSDP）分别对优化器状态、梯度、参数做分片；梯度累积用时间换 batch 大小；8-bit 优化器进一步压缩状态。推理侧：KV Cache 是增长主体，用量化（int8）、PagedAttention 分页、GQA/滑窗压缩；权重可 int4 量化减少驻留。',
    '面试官期待的是显存账本意识：能按 13B 模型粗算各部分占比（参数 26GB@bf16、Adam 状态约 104GB、激活随序列长度线性），再对号入座选手段。每个手段的代价也要能说清：梯度检查点约多 30% 计算、ZeRO-3 增加通信、量化有精度风险。'
  ),
  (
    'f1000000-0000-4000-8000-000000000042',
    '你了解哪些大模型的推理加速方法？（如 KV Cache、Flash Attention、PagedAttention 等）',
    'llm-inference-acceleration-methods',
    'knowledge', 'medium',
    'KV Cache 避免重复计算，FlashAttention 做 IO 感知的精确注意力，PagedAttention 分页管理 KV；再叠加量化、连续 batching、投机解码。',
    '主要手段：1）KV Cache 缓存历史 key/value，把每步复杂度从 O(n²) 降到 O(n)；2）FlashAttention 用分块和在线 softmax 减少 HBM 读写，是不损失精度的核级加速；3）PagedAttention（vLLM）把 KV Cache 按页管理，消除碎片、提高并发 batching 吞吐；4）量化（权重量化与 KV 量化）降低带宽；5）投机解码用小模型草拟、大模型验证；6）结构级改进如 GQA/滑窗直接缩小缓存。',
    '加分点是把「单请求延迟」和「集群吞吐」分开讨论：前者靠 FlashAttention/量化/投机解码，后者靠 PagedAttention + continuous batching。具身落点：VLA 在机载 GPU 上有硬实时预算，chunk 化输出 + 少步 Flow Matching + GQA 是 π 系列的组合拳。'
  ),
  (
    'f1000000-0000-4000-8000-000000000043',
    '大模型的对齐方法有哪些？（SFT、RLHF、DPO、GRPO 等）',
    'llm-alignment-methods-overview',
    'knowledge', 'medium',
    'SFT 打底，RLHF 用奖励模型 + PPO，DPO 直接从偏好对优化省掉奖励模型，GRPO 用组相对优势替代 critic；各有成本与稳定性取舍。',
    '主流路线：SFT 用高质量示范做监督微调，是对齐的地基；RLHF 先训练奖励模型再用 PPO 优化策略，效果好但工程复杂、易不稳定；DPO 从偏好对的排序关系推导出闭式损失，跳过显式奖励模型和在线采样，训练简单稳定；GRPO 对同一 prompt 采样一组输出、组内相对优势做策略梯度，去掉了 critic，在可验证奖励场景（数学、代码）广泛使用。此外还有 RLAIF、迭代式 DPO 等变体。',
    '回答时给出「成本-稳定性-上限」的对比框架：RLHF 上限高但需要同时维护 RM、策略与采样；DPO 离线简单但受偏好数据分布限制；GRPO 介于两者之间。具身对应关系值得主动建立：机器人版的 SFT 是演示模仿，RLHF 的对应物是基于人工纠正与可验证成功的后训练（如 RECAP）。'
  ),
  (
    'f1000000-0000-4000-8000-000000000044',
    '如何评估一个大模型的效果？常用的评测指标有哪些？',
    'how-to-evaluate-a-large-language-model',
    'knowledge', 'medium',
    '自动基准（知识、推理、代码）、困惑度、成对胜率与 LLM-as-judge、人类偏好竞技场，配合安全评测与数据污染检查。',
    '常用评估层次：1）自动基准——MMLU 类知识问答、GSM8K/MATH 类数学推理、HumanEval 类代码生成，按准确率/Pass@k 计分；2）语言建模质量用困惑度（perplexity）；3）开放生成用成对比较：人工标注胜率或 LLM-as-judge 自动打分；4）众包竞技场（Chatbot Arena）用真实用户投票换算 Elo；5）安全与鲁棒性专项评测。工程上还要控制数据污染（基准泄漏进训练集）和评测方差（温度、few-shot 设置）。',
    '能指出自动基准的局限（刷榜不代表真实可用、污染风险）并给出多维交叉验证的思路，是这题想听的。迁移到具身：同样逻辑对应成功率 + 泛化划分（新物体/新场景/新指令）+ 长时序完成度，可以顺势展示知识串联。'
  ),
  (
    'f1000000-0000-4000-8000-000000000045',
    '什么是 ACT？它的核心思想是什么？相比传统的模仿学习方法有哪些改进？',
    'what-is-act',
    'knowledge', 'medium',
    'ACT 用 CVAE 从示范中采样风格隐变量、Transformer 解码器一次输出一整段动作块，配合时间集成执行，缓解模仿学习的多模态与累积误差问题。',
    'ACT（Action Chunking Transformer，ALOHA 工作）是一个条件 VAE 策略：编码器把整段示范动作压缩成风格隐变量 z（捕捉同任务的不同解法），解码器以观测和 z 为条件，用 Transformer 一次预测一整段动作 chunk。执行时对重叠 chunk 做时间集成（加权平均）平滑衔接。相比朴素行为克隆的改进：隐变量让多模态示范不再被 MSE 平均成无效动作；chunk 化降低误差累积并摊薄推理频率。',
    '高频追问：为什么不用 diffusion 也能处理多模态（CVAE 隐变量 vs 迭代去噪两条路线）、chunk 长度与时间集成系数怎么调、双臂高精度任务（ALOHA）里它为什么有效。与 π0 的 action chunk 设计可以对比着讲。'
  ),
  (
    'f1000000-0000-4000-8000-000000000046',
    '请讲一下 DAgger 的基本思想？它解决了什么问题？在具身智能中如何应用？',
    'what-is-dagger',
    'knowledge', 'medium',
    'DAgger 迭代地用当前策略滚动、让专家在策略访问的状态上标注，聚合数据重训，解决行为克隆的分布偏移（covariate shift）。',
    '行为克隆只在专家状态分布上训练，策略一旦犯错进入未见状态就会雪崩——这是 covariate shift。DAgger 的解法：用当前策略与环境交互，把访问到的状态交给专家标注正确动作，把新标注聚合进数据集后重训策略，反复迭代直到策略覆盖自己的错误分布。理论上它把学习目标从「模仿专家在专家轨迹上的动作」修正为「在策略自身状态分布上也做对」。',
    '具身应用与代价要一起说：真机上「专家标注」= 人类遥操作纠正或共享自治（人只接管错误状态），这正是纠正式数据闭环（如 RECAP 的人工纠正段）的理论前身；代价是专家必须实时在线、标注延迟可能破坏状态分布（用 DAgger-by-Proxy 或事后标注缓解）。'
  ),
  (
    'f1000000-0000-4000-8000-000000000047',
    'LeRobot 格式数据是什么？它的主要组成部分和优势有哪些？',
    'lerobot-dataset-format',
    'knowledge', 'medium',
    'Hugging Face 机器人社区的标准化数据集格式：Parquet 表 + 视频 episode、统一观测/动作空间元数据，可流式加载与跨本体复用。',
    'LeRobot 格式由 Hugging Face 机器人团队提出，把机器人操作数据组织为：元信息（本体、频率、任务标签、episode 索引）+ Parquet 表存储状态/动作等数值序列 + mp4 视频按帧索引存储相机流。配套 dataloader 可像 torchvision 数据集一样按 episode/帧流式读取，天然对接 Hugging Face Hub 的托管与版本化。',
    '它的价值在于把「数据集契约」标准化：统一的观测/动作空间声明使跨本体训练和社区共享变得现实，也便于做数据质量审查与可视化。回答时能对比 RLDS/TFDS、Open X Embodiment 的格式演化史是加分项。'
  ),
  (
    'f1000000-0000-4000-8000-000000000048',
    'UMI 数据集是什么？它的特点和应用场景是什么？',
    'umi-dataset',
    'knowledge', 'medium',
    'UMI 用手持夹爪 + GoPro + SLAM 恢复轨迹，不依赖机器人就能在真实场景大规模采集操作示范，再迁移到真机策略。',
    'UMI（Universal Manipulation Interface，斯坦福 2024）是一套采集接口：操作员手持装夹爪和广角相机的支架完成任务，离线用 SLAM 恢复末端轨迹与夹爪开合时序，配合时延校正和相对轨迹表示，把人类演示转成可训练的操作数据集。特点是采集成本低、场景自由（家庭、野外）、完全不占用机器人机时。',
    '关键取舍要讲清楚：没有机器人本体感知，策略只能依赖视觉与恢复的相对位姿，因此配合扩散策略类模型在真实机械臂上也能迁移；对高精度力控任务不如真机遥操作数据。它与遥操作数据（ALOHA）、仿真数据构成具身数据的三大来源，适合被追问「什么任务该用哪种」。'
  ),
  (
    'f1000000-0000-4000-8000-000000000049',
    '具身智能中常见的数据增强方法有哪些？它们对模型泛化能力的提升效果如何？',
    'embodied-data-augmentation-methods',
    'knowledge', 'medium',
    '视觉域（裁剪/颜色/遮挡）、域随机化（光照纹理相机）、视角与时间增强、动作扰动等；对视觉泛化提升明显，但必须避免破坏动作语义。',
    '常用增强：视觉层——随机裁剪、颜色/亮度抖动、遮挡与噪点；场景层——域随机化（纹理、光照、物体摆放、相机位姿随机化），仿真里尤其常用；视角层——多相机重采样与图像变换后同步修正动作坐标；时间层——加减速、丢帧插值；动作层——小幅扰动与平滑。对泛化的实际效果：视觉与光照随机化对新环境/新物体外观的提升最稳定， viewpoint 增强提升相机安装变化的鲁棒性，动作扰动过多反而损害精细操作。',
    '回答要点是「增强不能改变动作标签的正确性」：比如相机移动必须同步变换动作坐标系，否则增强即投毒。能结合π0.5 的 co-training、域随机化的 sim2real 角色一起讲会显得体系完整。'
  ),
  (
    'f1000000-0000-4000-8000-000000000050',
    '在具身智能任务中，如何设计奖励函数？有哪些典型的奖励设计思路？',
    'reward-design-for-embodied-tasks',
    'knowledge', 'hard',
    '从稀疏任务成功奖励到势函数塑形、课程奖励、学习型奖励（分类器/视觉 reward model/时间一致性自举），核心权衡是塑形收益与奖励黑客风险。',
    '典型思路按监督密度递增：1）稀疏奖励——只用任务成功/失败，无偏但探索难；2）势函数塑形（potential-based shaping）——以距离/子任务进度给密集信号，且理论上不改变最优策略；3）课程奖励——随训练进度逐步收紧容差或增加难度；4）学习型奖励——用成败分类器、视觉 reward model 或时间一致性自举从少量标注扩展到全量数据（π*0.6 的做法）；5）偏好奖励——人类偏好比较训练 reward model。',
    '这题的高分点在风险意识：稠密手工奖励容易被黑客（机器人学会蹭奖励而不是完成任务），所以现代机器人后训练普遍「稀疏成功 + 学习型 value + 人工纠正兜底」。能对比仿真（可以放手塑形）与真机（信号只能来自少量人工核验）的不同约束更佳。'
  ),
  (
    'f1000000-0000-4000-8000-000000000051',
    '具身智能中的 RTC 相关概念有哪些？（实时控制、延迟、控制频率等）它们对系统性能有什么影响？',
    'real-time-control-concepts',
    'knowledge', 'medium',
    '控制频率、端到端延迟、抖动与确定性、内外环分工是核心概念；感知-推理-执行的延迟链直接决定系统的稳定性与反应能力。',
    '关键概念：控制频率——伺服内环常在 kHz 级，策略外环 10-50Hz；延迟链——感知采集、传输、模型推理、执行各段延迟之和决定响应带宽；抖动——推理时间的方差比均值更伤稳定性，会引发动作衔接毛刺；内外环分工——高频低层（阻抗/位置控制）兜底，低频高层（VLA）出目标轨迹；执行缓冲——动作 chunk 按时间戳回放，配合插值与外推隐藏推理延迟。',
    '对性能的影响要落到现象上：延迟过大会震荡和过冲，抖动会造成动作卡顿，chunk 边界处理不当产生频繁顿挫——这正是 π0 用 action extrapolation、ACT 用时间集成的原因。能画出「感知→推理→执行」的延迟预算图并指出瓶颈位置，就是完整答案。'
  ),
  (
    'f1000000-0000-4000-8000-000000000052',
    '在真实机器人部署中，常见的工程挑战有哪些？如何解决 sim2real 的问题？',
    'deployment-challenges-and-sim2real',
    'knowledge', 'medium',
    '部署挑战覆盖算力/延迟、标定漂移与安全兜底；Sim2Real 差距来自物理、视觉与执行器建模，靠域随机化、系统辨识和真机微调弥合。',
    '部署工程挑战：机载算力与实时预算、传感器标定漂移与时间同步、长期运行的故障恢复与安全包络（力/速度限制、急停）、数据回传与在线学习闭环。Sim2Real 差距来源：1）物理——接触动力学、摩擦、执行器响应建模不准；2）视觉——渲染真实感不足；3）系统——延迟与控制频率不一致。主流缓解：域随机化（让真实世界成为随机化分布内的一次采样）、系统辨识校准动力学参数、高保真 GPU 并行仿真（Isaac 等）、真机少量微调与教师-学生蒸馏。',
    '回答结构建议「差距来源 → 对症下药」：视觉差距用随机化+真实数据混合，动力学差距用辨识+重仿真，策略层面用真机数据闭环收尾。能举出现代流程（仿真预训练 → 域随机化 → 真机 RECAP 微调）说明各方法的适用位置。'
  ),
  (
    'f1000000-0000-4000-8000-000000000053',
    '你了解哪些具身智能的评测基准？它们分别评估什么能力？',
    'embodied-evaluation-benchmarks',
    'knowledge', 'medium',
    '仿真操作基准有 LIBERO、Meta-World、RLBench、CALVIN、RoboCasa 等，分别考察持续学习、多任务、长时序语言跟随等能力；RT-1/RT-2 是模型而非基准。',
    '常用仿真基准：LIBERO——面向知识迁移与终身学习的操作任务套件（本体/物体/目标/关系四种泛化划分）；Meta-World——50 个多任务操作环境，考察多任务 RL 与泛化；RLBench——高自由度臂上的大规模任务集；CALVIN——长时序语言条件任务链，考察语言跟随与组合性；RoboCasa——家庭场景大规模仿真。真实评测多以自建任务集 + 成功率报告；Open X Embodiment 提供跨本体评测协议，SimplerEnv 尝试用仿真代理真实评测。注意 RT-1/RT-2 是模型，其任务集常被借用但本身不是基准。',
    '高分回答会按「考察维度」组织：多任务泛化（Meta-World）、持续学习（LIBERO）、长时序语言（CALVIN）、场景规模（RoboCasa），并指出仿真成功率与真机表现仍有显著相关性缺口——这也是为什么 π 系列论文坚持真机验证。'
  );
...
-- π 系列 24 题 (primary topic: π 系列; secondaries point at the underlying concept)
insert into public.question_topics (question_id, topic_id, weight) values
  ('f1000000-0000-4000-8000-000000000011', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000011', 'd1000000-0000-4000-8000-000000000006', 0.8),

  ('f1000000-0000-4000-8000-000000000012', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000012', 'd1000000-0000-4000-8000-000000000016', 0.9),
  ('f1000000-0000-4000-8000-000000000012', 'd1000000-0000-4000-8000-000000000006', 0.7),

  ('f1000000-0000-4000-8000-000000000013', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000013', 'd1000000-0000-4000-8000-000000000017', 0.9),
  ('f1000000-0000-4000-8000-000000000013', 'd1000000-0000-4000-8000-000000000006', 0.5),

  ('f1000000-0000-4000-8000-000000000014', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000014', 'd1000000-0000-4000-8000-000000000006', 0.9),

  ('f1000000-0000-4000-8000-000000000015', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000015', 'd1000000-0000-4000-8000-000000000006', 0.7),

  ('f1000000-0000-4000-8000-000000000016', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000016', 'd1000000-0000-4000-8000-000000000006', 0.6),

  ('f1000000-0000-4000-8000-000000000017', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000017', 'd1000000-0000-4000-8000-000000000006', 0.8),

  ('f1000000-0000-4000-8000-000000000018', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000018', 'd1000000-0000-4000-8000-000000000006', 0.7),

  ('f1000000-0000-4000-8000-000000000019', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000019', 'd1000000-0000-4000-8000-000000000006', 0.7),
  ('f1000000-0000-4000-8000-000000000019', 'd1000000-0000-4000-8000-000000000013', 0.5),

  ('f1000000-0000-4000-8000-000000000020', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000020', 'd1000000-0000-4000-8000-000000000006', 0.8),

  ('f1000000-0000-4000-8000-000000000021', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000021', 'd1000000-0000-4000-8000-000000000009', 0.8),
  ('f1000000-0000-4000-8000-000000000021', 'd1000000-0000-4000-8000-000000000010', 0.6),

  ('f1000000-0000-4000-8000-000000000022', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000022', 'd1000000-0000-4000-8000-000000000013', 0.7),
  ('f1000000-0000-4000-8000-000000000022', 'd1000000-0000-4000-8000-000000000009', 0.6),

  ('f1000000-0000-4000-8000-000000000023', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000023', 'd1000000-0000-4000-8000-000000000009', 0.8),

  ('f1000000-0000-4000-8000-000000000024', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000024', 'd1000000-0000-4000-8000-000000000007', 0.7),
  ('f1000000-0000-4000-8000-000000000024', 'd1000000-0000-4000-8000-000000000006', 0.6),

  ('f1000000-0000-4000-8000-000000000025', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000025', 'd1000000-0000-4000-8000-000000000009', 0.9),

  ('f1000000-0000-4000-8000-000000000026', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000026', 'd1000000-0000-4000-8000-000000000009', 0.9),

  ('f1000000-0000-4000-8000-000000000027', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000027', 'd1000000-0000-4000-8000-000000000011', 0.8),
  ('f1000000-0000-4000-8000-000000000027', 'd1000000-0000-4000-8000-000000000009', 0.7),

  ('f1000000-0000-4000-8000-000000000028', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000028', 'd1000000-0000-4000-8000-000000000009', 0.8),
  ('f1000000-0000-4000-8000-000000000028', 'd1000000-0000-4000-8000-000000000010', 0.7),

  ('f1000000-0000-4000-8000-000000000029', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000029', 'd1000000-0000-4000-8000-000000000018', 0.9),
  ('f1000000-0000-4000-8000-000000000029', 'd1000000-0000-4000-8000-000000000016', 0.6),

  ('f1000000-0000-4000-8000-000000000030', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000030', 'd1000000-0000-4000-8000-000000000018', 1.0),

  ('f1000000-0000-4000-8000-000000000031', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000031', 'd1000000-0000-4000-8000-000000000018', 0.8),

  ('f1000000-0000-4000-8000-000000000032', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000032', 'd1000000-0000-4000-8000-000000000006', 0.6),

  ('f1000000-0000-4000-8000-000000000033', 'd1000000-0000-4000-8000-000000000015', 1.0),

  ('f1000000-0000-4000-8000-000000000034', 'd1000000-0000-4000-8000-000000000015', 1.0),
  ('f1000000-0000-4000-8000-000000000034', 'd1000000-0000-4000-8000-000000000006', 0.8);

-- 大模型八股 + 具身高频补充 19 题
insert into public.question_topics (question_id, topic_id, weight) values
  ('f1000000-0000-4000-8000-000000000035', 'd1000000-0000-4000-8000-000000000003', 1.0),
  ('f1000000-0000-4000-8000-000000000035', 'd1000000-0000-4000-8000-000000000002', 0.6),

  ('f1000000-0000-4000-8000-000000000036', 'd1000000-0000-4000-8000-000000000003', 1.0),
  ('f1000000-0000-4000-8000-000000000036', 'd1000000-0000-4000-8000-000000000005', 0.8),

  ('f1000000-0000-4000-8000-000000000037', 'd1000000-0000-4000-8000-000000000002', 1.0),

  ('f1000000-0000-4000-8000-000000000038', 'd1000000-0000-4000-8000-000000000019', 1.0),
  ('f1000000-0000-4000-8000-000000000038', 'd1000000-0000-4000-8000-000000000006', 0.5),

  ('f1000000-0000-4000-8000-000000000039', 'd1000000-0000-4000-8000-000000000020', 1.0),

  ('f1000000-0000-4000-8000-000000000040', 'd1000000-0000-4000-8000-000000000016', 1.0),
  ('f1000000-0000-4000-8000-000000000040', 'd1000000-0000-4000-8000-000000000008', 0.7),

  ('f1000000-0000-4000-8000-000000000041', 'd1000000-0000-4000-8000-000000000020', 1.0),

  ('f1000000-0000-4000-8000-000000000042', 'd1000000-0000-4000-8000-000000000005', 1.0),
  ('f1000000-0000-4000-8000-000000000042', 'd1000000-0000-4000-8000-000000000002', 0.6),

  ('f1000000-0000-4000-8000-000000000043', 'd1000000-0000-4000-8000-000000000021', 1.0),
  ('f1000000-0000-4000-8000-000000000043', 'd1000000-0000-4000-8000-000000000010', 0.5),
  ('f1000000-0000-4000-8000-000000000043', 'd1000000-0000-4000-8000-000000000011', 0.5),

  ('f1000000-0000-4000-8000-000000000044', 'd1000000-0000-4000-8000-000000000024', 1.0),

  ('f1000000-0000-4000-8000-000000000045', 'd1000000-0000-4000-8000-000000000022', 1.0),
  ('f1000000-0000-4000-8000-000000000045', 'd1000000-0000-4000-8000-000000000017', 0.8),

  ('f1000000-0000-4000-8000-000000000046', 'd1000000-0000-4000-8000-000000000022', 1.0),
  ('f1000000-0000-4000-8000-000000000046', 'd1000000-0000-4000-8000-000000000009', 0.5),

  ('f1000000-0000-4000-8000-000000000047', 'd1000000-0000-4000-8000-000000000013', 1.0),

  ('f1000000-0000-4000-8000-000000000048', 'd1000000-0000-4000-8000-000000000013', 1.0),

  ('f1000000-0000-4000-8000-000000000049', 'd1000000-0000-4000-8000-000000000013', 1.0),

  ('f1000000-0000-4000-8000-000000000050', 'd1000000-0000-4000-8000-000000000009', 1.0),

  ('f1000000-0000-4000-8000-000000000051', 'd1000000-0000-4000-8000-000000000012', 1.0),
  ('f1000000-0000-4000-8000-000000000051', 'd1000000-0000-4000-8000-000000000017', 0.7),

  ('f1000000-0000-4000-8000-000000000052', 'd1000000-0000-4000-8000-000000000023', 1.0),
  ('f1000000-0000-4000-8000-000000000052', 'd1000000-0000-4000-8000-000000000012', 0.6),

  ('f1000000-0000-4000-8000-000000000053', 'd1000000-0000-4000-8000-000000000024', 1.0);
